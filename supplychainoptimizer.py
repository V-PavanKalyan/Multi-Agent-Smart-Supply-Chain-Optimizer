...
if STATIC_MODE: # Check if the system is in STATIC_MODE
    print("🔧 Running in STATIC MODE (CSV data only, no API calls)") # Print a message indicating STATIC_MODE
else: # If not in STATIC_MODE (meaning it's in DYNAMIC_MODE)
    print("🌐 Running in DYNAMIC MODE (with live API integrations)") # Print a message indicating DYNAMIC_MODE
...
print("🤖 LLM initialized successfully!") # Print a message indicating successful LLM initialization
...
class AgentMemory: # Define a class named AgentMemory to manage agent memories
    ...
    def add_message(self, from_agent: str, to_agent: str, payload: Dict): # Define a method to add an inter-agent message to memory
        """🆕 Add inter-agent message to memory""" # Add a docstring explaining the purpose of the method
...
print("🧠 Memory management system initialized!") # Print a message indicating that the memory management system has been initialized
...
class SupplyChainTestFramework: # Define a class named SupplyChainTestFramework
    ...
    def run_test_scenario(self, scenario_name: str, test_data: Dict, expected_outcomes: List[str]) -> Dict: # Define a method to run a single test scenario
        ...
        print(f"\n🧪 Running Test: {scenario_name}") # Print a message indicating the start of the test scenario
        ...
        try: # Start a try block to handle potential errors during the test
            ...
            status_emoji = "✅" if test_result["status"] == "PASS" else "❌" # Determine the appropriate emoji based on the test status
            print(f"{status_emoji} {scenario_name}: {test_result['status']}") # Print the test scenario name and status
            print(f"⏱️  Execution time: {execution_time:.2f}s") # Print the execution time
            print(f"✅ Passed: {len(passed_checks)}/{len(expected_outcomes)}") # Print the number of passed checks out of the total expected outcomes

            if failed_checks: # Check if there are any failed checks
                print(f"❌ Failed checks: {', '.join(failed_checks)}") # Print the list of failed checks
            ...
            print(f"❌ {scenario_name}: ERROR - {str(e)}") # Print an error message
    ...
    def run_all_tests(self) -> Dict: # Define a method to run all test scenarios
        ...
        print("🚀 Starting Supply Chain Optimizer Test Suite") # Print a message indicating the start of the test suite
        ...
        print("📊 TEST SUITE COMPLETE") # Print a message indicating the test suite is complete
        print(f"✅ Passed: {summary['passed']}/{summary['total']}") # Print the number of passed tests
        print(f"❌ Failed: {summary['failed']}/{summary['total']}") # Print the number of failed tests
        print(f"⏱️  Total time: {summary['total_time']:.2f}s") # Print the total execution time
        print("📄 Results saved to test_results.json") # Confirm that the results have been saved
...
print("🧪 Test framework ready!") # Print a message indicating that the test framework is ready
...
print("📊 Visualization components ready!") # Print a message indicating that the visualization components are ready

def main_pipeline():
    """Main execution pipeline for the supply chain optimizer"""

    print("🚀 MULTI-AGENT SMART SUPPLY CHAIN OPTIMIZER") # Print a header for the pipeline
    print("=" * 60) # Print a separator line
    ...
    print("\n📊 INITIAL DATA OVERVIEW (using updated locations)") # Print a header for the initial data overview
    ...
    print("\n📈 GENERATING VISUALIZATIONS") # Print a header for the visualization section
    print("-" * 30) # Print a separator line
    ...
    print("\n🤖 RUNNING INITIAL ANALYSIS") # Print a header for the initial analysis section
    print("-" * 30) # Print a separator line
    ...
    print("\n📋 ANALYSIS RESULTS:") # Print a header for the analysis results
    ...
    if "route" in planner.shared_scratchpad["agent_outputs"]: # Check if route optimization results are available in the planner's scratchpad
        ...
        if locations_for_map: # Check if there are locations available for the map
            ...
            if route_map: # Check if the route map was successfully created
                ...
                print("📍 Route map saved as 'supply_chain_route_map.html'") # Confirm that the map was saved
    ...
    print("\n🧪 RUNNING COMPREHENSIVE TEST SUITE") # Print a header for the test suite section
    print("-" * 30) # Print a separator line

    test_summary = test_framework.run_all_tests() # Run all test scenarios using the test framework

    print("\n📄 GENERATING FINAL OUTPUT") # Print a header for the final output section
    print("-" * 30) # Print a separator line
    ...
    print("\n✅ PIPELINE COMPLETE!") # Indicate that the pipeline has completed successfully
...
def generate_human_explanation(agent_outputs: Dict) -> str: # Define a function to create a human-readable explanation
    ...
    explanation = "🔍 SUPPLY CHAIN ANALYSIS SUMMARY\n\n" # Initialize the explanation string with a header

    if "inventory" in agent_outputs: # Check if inventory analysis results are available
        inv = agent_outputs["inventory"] # Get the inventory analysis data
        explanation += f"📦 INVENTORY: Analyzed {inv['total_items']} items. " # Add inventory analysis summary
    ...
    if "supplier" in agent_outputs and agent_outputs["supplier"].get("recommended_suppliers"): # Check if supplier recommendations are available
        sup_data = agent_outputs["supplier"]["recommended_suppliers"] # Get the recommended suppliers data
        if sup_data: # Check if there is any supplier data
            explanation += f"🏭 SUPPLIERS: Evaluated suppliers for critical items. " # Add supplier analysis summary
    ...
    if "route" in agent_outputs: # Check if route optimization results are available
        route = agent_outputs["route"] # Get the route optimization data
        explanation += f"🚚 LOGISTICS: Optimized route covers {route['total_distance_km']} km " # Add route optimization summary (distance)
    ...
    if "risk" in agent_outputs: # Check if risk analysis results are available
        risk = agent_outputs["risk"] # Get the risk analysis data
        explanation += f"⚠️  RISKS: Overall risk level assessed as {risk['overall_risk_level'].upper()}. " # Add risk analysis summary (overall risk level)
        explanation += f"Primary concerns include weather conditions and potential supplier delays.\n\n" # Add primary risk concerns

    explanation += "💡 The system has analyzed all aspects of your supply chain and provided optimized recommendations balancing cost, time, and risk factors." # Add a concluding sentence
...
def generate_human_final_plan(agent_outputs: Dict) -> str: # Define a function to create a human-readable final plan
    ...
    if critical_items > 0: # Check if there are any critical items
        plan += f"🔥 IMMEDIATE ACTIONS:\n" # Add a header for immediate actions
        plan += f"• Expedite restocking of {critical_items} critical items\n" # Add an action to expedite restocking
        plan += f"• Contact recommended suppliers for emergency orders\n" # Add an action to contact recommended suppliers
        plan += f"• Implement express shipping for fastest delivery\n\n" # Add an action to implement express shipping

    plan += f"📋 STRATEGIC RECOMMENDATIONS:\n" # Add a header for strategic recommendations

    if "supplier" in agent_outputs and agent_outputs["supplier"].get("recommended_suppliers"): # Check if supplier recommendations are available
        plan += f"• Utilize recommended suppliers for critical items.\n" # Add a recommendation to use recommended suppliers

    if "route" in agent_outputs: # Check if route optimization results are available
        route = agent_outputs["route"] # Get the route optimization data
        plan += f"• Follow optimized {route['total_distance_km']} km route for cost efficiency\n" # Add a recommendation to follow the optimized route (distance)
        plan += f"• Schedule deliveries within {route['estimated_duration_hours']:.0f} hour window\n" # Add a recommendation to schedule deliveries (duration)


    if "risk" in agent_outputs: # Check if risk analysis results are available
        risk = agent_outputs["risk"] # Get the risk analysis data
        plan += f"• Monitor {risk['overall_risk_level']} risk conditions closely\n" # Add a recommendation to monitor risk conditions
        plan += f"• Maintain contingency plans for weather disruptions\n" # Add a recommendation to maintain contingency plans

    plan += f"\n🎯 This plan balances delivery speed, cost optimization, and risk mitigation to ensure reliable supply chain operations." # Add a concluding sentence for the plan
...
def generate_structured_plan(agent_outputs: Dict) -> str:
    ...
    return " → ".join(actions)  # Join all action steps with an arrow to form the final structured plan string
...
def save_session_data(planner: PlannerManagerAgent, test_summary: Dict, final_output: Dict): # Define a function to save session data to files
    ...
    print(f"💾 All data saved with timestamp: {timestamp}") # Print a confirmation message indicating that all data has been saved with the timestamp
...
if __name__ == "__main__":
    try: # Start a try block to handle potential errors during the pipeline execution
        ...
        print("\n🎉 Supply Chain Optimizer completed successfully!") # Print a success message if the pipeline completes without errors

    except Exception as e: # Catch any exceptions that occur during the pipeline execution
        print(f"\n❌ Error in main pipeline: {str(e)}") # Print an error message to the console
...
def interactive_demo():
    ...
    bus = CommunicationBus()  # Initialize the communication bus for agents to talk to each other
    planner = PlannerManagerAgent(llm, bus)   # Initialize the main planner agent, which orchestrates the other agents
    print("\n🎮 INTERACTIVE SUPPLY CHAIN OPTIMIZER DEMO")   # Print a welcoming message for the interactive demo
    print("=" * 50)
    ...
    while True:        # Start an infinite loop to keep the demo running until the user quits
        try:       # Prompt the user for input and remove leading/trailing whitespace
            ...
            if user_input.lower() == 'quit':     # Check if the user wants to quit the demo
                print("👋 Goodbye!")   # Print a goodbye message and break out of the loop
                break
            ...
            print("\n🤖 Planner Agent: Processing your request...")
...
• memory    - Show agent memory summaries
...
• "Why was supplier X recommended?"
...
print("\n🎯 Supply Chain Optimizer is ready!")
